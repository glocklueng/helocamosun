...
/****************************************************************
*
*	This the the new X-Cell helicopter simulation model.
*
*	Author: Aaron Kahn
*
****************************************************************/
...
struct mainrotor_def {
	...
	double	b1;						// laterial TPP deflection wrt hub (+ right tilt rad)
	double	kc;						// flapping stiffness
	double	a1;						// longitudinal TPP deflection wrt hub (+ back tilt rad)
	double	db1dv;					// TPP lateral dihedral effect derivative
	double	b_sum;					// used in MR TPP dynamics
	double	da1du;					// TPP longitudinal dihedral effect derivative
	double	a1dot;					// used in MR TPP dynamics (longitudinal dynamics)
	double	b1dot;					// used in MR TPP dynamics (lateral dynamics)
	...
	double	a;						// blade lift curve slope (Clalpha */rad)
	...
	double	dl_db1;					// Rolling moment due to b1 TPP tilt derivative (lb-ft/rad)
	...
	double	e;						// MR equivelent hinge offset (ft)
	double	i_b;					// MR blade inertial about root (slug-ft^2)
	double	cd0;					// profile drag cooeficent for a MR blade
	double	ct;						// MR thrust cooeficent
	...
	double k2;						// cross coupling cooefficent
	double w_in;					// inplane change in flap
	double w_off;					// off-plane change in flap
	double tau;						// TPP time constant (sec)
...
struct tailrotor_def{
	...
	double	a;						// TR blade lift curve slope (Clalpha */rad)
	...
	double	cd0;					// TR blade profile drag cooeficent
	double	fr;						// normalized TR drag
	double	duct;					// factor to correct for duct augmetation
};

// Fly Bar (control rotor) Parameters (based on Tischler and Mettler)
struct flybar_def {
	double tau;						// flybar time constant (sec)
	double d;						// lateral flybar TPP tilt (rad)
	double d_dot;					// lateral flybar TPP tilt rate (rad/s)
	double c;						// longitudinal flybar TPP tilt (rad)
	double c_dot;					// longitudinal flybar TPP tilt rate (rad/s)
...
struct verticalfin_def {
	...
	double	yuv;					// horzontal-side flat plate drag area (ft^2)
	...
	double	wl;						// fin vertical waterling point (from MR hub in)
	double	a;						// fin lift curve slope (Clalpha */rad)
};

// Horizontal Fin Parameters
struct horizontalfin_def {
	double	d_dw;					// downwash impinging on tail
	double	h;						// vertical distance of fin to aircraft CG (ft)
	double	d;						// horizontal distance of fin to aircraft CG (ft)
	double	eps;					
...
// Fuselage Parameters
struct fuse_def {
	double	wa;						// rotor downwash on fuselage
...
struct control_def {
	...
	double A1;						// A1 swashplate angle = roll cyclic pitch (rad + right roll)
	double B1;						// B1 swashplate angle = pitch cyclic pitch (rad + nose down)
	double tr_rev;					// tail rotor RPM (rpm)
	double mr_rev;					// main rotor RPM (rpm)
	double gyro_gain;				// tail rotor hobby gyro gain
	int wow;						// weight-on-wheel switch (1=on ground, 0=in air)
};

// CG Parameters
struct cg_def {
	double F[MAXSIZE];					// F[X Y Z] forces acting on CG (lb)
	double M[MAXSIZE];					// M[L M N] moments acting about CG (lb-ft)
	double THETA[MAXSIZE];				// THETA[phi theta psi] euler body angles (rad)
	double pqr[MAXSIZE];				// pqr[p q r] body angular rates (rad/s)
	double uvw[MAXSIZE];				// uvw[u v w] velocity in body axis (ft/s)
	double V[MAXSIZE];					// V[velN velE velD] velocity in TP NED axis (ft/s)
	double NED[MAXSIZE];				// NED[North East Down] position (ft)
...
struct blade_element_def {
	...
	double Cd0;							// profile drag cooeficent (nondimentional)
	double e;							// oswald efficency factor (nondimentional)
...
extern struct heli_def xcell;

/********** FUNCTIONS ******************/

extern double model_dt;		// This is the integration time step for the propogation of the model.
...
* propogate the helicopter model, 6-DOF, landing gear, servos.
*
